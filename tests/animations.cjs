...
for(const ms of [0,60,120,240,800]){
 ...
 v.contacts.forEach((h,i)=>{const r=limbRoot(v.body,i),j=limbJoint(r,h,i),half=LIMB_LENGTHS[i]/2;assert(Math.abs((half-distance(r,j))-8*(half-distance(h,j)))<1e-5,'Settle changes shortening ratio');});
}
...
for(const ms of [0,160,325,650,1000,2000]){
 ...
 v.contacts.forEach((h,i)=>{const r=limbRoot(v.body,i),j=limbJoint(r,h,i),half=LIMB_LENGTHS[i]/2;assert(Math.abs((half-distance(r,j))-8*(half-distance(h,j)))<1e-5,'Hang changes shortening ratio');});
}
